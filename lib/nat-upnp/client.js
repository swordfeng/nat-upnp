...
Client.prototype.getMappings = function getMappings(options, callback) {
  ...
  this.findGateway(function(err, gateway, gwAddress, address) {
    if (err) return callback(err);
    var i = 0;
    var end = false;
    var results = [];
...
Client.prototype.findGateway = function findGateway(callback) {
  ...
  p.on('device', function (info, gwAddress, clientAddress) {
    if (timeouted) return;
    p.emit('end');
    clearTimeout(timeout);

    // Create gateway
    callback(null, nat.device.create(info.location), gwAddress, clientAddress);
  });
};
